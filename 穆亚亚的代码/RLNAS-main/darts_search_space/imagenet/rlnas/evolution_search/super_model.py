...
class NetworkImageNet(nn.Module):
    ...
    def forward(self, input, normal_cell, reduction_cell):
        s0 = self.stem0(input)
        s1 = self.stem1(s0)
        for i, cell in enumerate(self.cells):
            if cell.reduction == True:
                s0, s1 = s1, cell(s0, s1, reduction_cell)
            else:
                s0, s1 = s1, cell(s0, s1, normal_cell)
    ...
    def insert_layer(self):
        ...
        cell = Cell(self._steps, self._multiplier, C_prev_prev, C_prev, C_curr, False, reduction_prev)
...
if __name__ == '__main__':
    ...
    norm_rng = [np.random.randint(len(config.blocks_keys)) for i in range(config.edges)]
    reduction_rng = [np.random.randint(len(config.blocks_keys)) for i in range(config.edges)]
    norm_rng = check_cand(norm_rng, operations)
    reduction_rng = check_cand(reduction_rng, operations)
    x = torch.rand(1, 3, 224, 224)
    logit = model(x, norm_rng, reduction_rng)
    print('logit:{0}, logit:{1}'.format(logit, logit.size()))
    from  thop import profile
    macs, params = profile(model, input_size=(1,3,224,224), normal_cell=norm_rng, reduction_cell=reduction_rng)

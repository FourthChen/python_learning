...
#     r = requests.get(url, headers=headers,verify=False)  # 增加headers, 模拟浏览器
#     return r
#
#
...
#         save_txt(output.format(page, author, vote, comment, content))
#
#
# def save_txt(*args):
#
...
urllib3.disable_warnings()
for j in range(15):
    url='https://www.qiushibaike.com/text/page/%s/'%j
    headers = {
    ...
    reg=re.compile('<h2>(?P<name>.*?)</h2>.*?<div class="articleGender manIcon">(?P<age>.*?)</div>',re.S)
